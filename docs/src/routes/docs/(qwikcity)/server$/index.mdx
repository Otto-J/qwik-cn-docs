...
```tsx
export const getEnvVariable = server$(function () {
...
export const handleCookies = server$(function () {
  ...
  if (!userSession) {
    this.cookie.set('user-session', 'new-session-id', {
      path: '/',
      httpOnly: true,
    });
  }
...
```tsx
export const getRequestUrl = server$(function () {
...
```tsx
export const getHeaders = server$(function () {
...
```tsx
export const handleRequest = server$(function () {
